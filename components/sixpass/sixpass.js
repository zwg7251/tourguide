      ...
      observer: function (newVal, oldVal) {
        ...
        if (newVal == 6) {
          ...
          setTimeout(function () {
            ...
            if (input_value == getApp().data.riskCode) {
              that.triggerEvent('sendRiskmsg', input_value)
            }
            else if(input_value == getApp().data.securityCode){
              that.triggerEvent('sendSecmsg', input_value)
            }else{
              that.triggerEvent('codeError',input_value)
            }
            // 当没有
            if (!that.data.isNext) {
              // 回到初始样式
